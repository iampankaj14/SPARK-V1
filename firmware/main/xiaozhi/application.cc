...
void Application::ActivationTask() {
    ...
    // Initialize the communication protocol first so voice chat is ready immediately
    InitializeProtocol();
...
void Application::InitializeProtocol() {
    ...
    protocol_->OnIncomingJson([this, display](const cJSON* root) {
        ...
        if (strcmp(type->valuestring, "tts") == 0) {
            ...
            ESP_LOGE(TAG, "Server returned error: %s", json_str);
            auto message = cJSON_GetObjectItem(root, "message");
            auto text = cJSON_GetObjectItem(root, "text");
            std::string err_msg = message && cJSON_IsString(message) ? message->valuestring : 
                                 (text && cJSON_IsString(text) ? text->valuestring : json_str);
            Schedule([this, display, err_msg]() {
                display->SetChatMessage("system", err_msg.c_str());
                display->ShowNotification(err_msg.c_str());
                Alert(Lang::Strings::ERROR, err_msg.c_str(), "cloud_off", Lang::Sounds::OGG_EXCLAMATION);
            });
            cJSON_free(json_str);
        } else {
...
void Application::HandleStartListeningEvent() {
    ...
    if (state == kDeviceStateIdle) {
        ...
        // Tapping the button while listening stops listening immediately and triggers AI reply
        HandleStopListeningEvent();
        return;
    }
}
