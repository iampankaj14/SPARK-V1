...
bool WebsocketProtocol::OpenAudioChannel() {
    ...
    ESP_LOGI(TAG, "WS Auth: token='%s', Device-Id='%s', Client-Id='%s', Proto-Ver=%d",
             token.c_str(), SystemInfo::GetMacAddress().c_str(),
             Board::GetInstance().GetUuid().c_str(), version_);
    ...
    websocket_->OnData([this](const char* data, size_t len, bool binary) {
        if (binary) {
            ...
            ESP_LOGI(TAG, "<< %s", debug_json ? debug_json : "(null)");
            if (debug_json) cJSON_free(debug_json);
