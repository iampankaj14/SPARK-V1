...
bool MqttProtocol::StartMqttClient(bool report_error) {
    ...
    ESP_LOGI(TAG, "NVS MQTT Settings:");
    ESP_LOGI(TAG, "  endpoint: %s", endpoint.c_str());
    ESP_LOGI(TAG, "  client_id: len=%d, val=%s...", (int)client_id.length(), client_id.substr(0, std::min((size_t)5, client_id.length())).c_str());
    ESP_LOGI(TAG, "  username: len=%d, val=%s...", (int)username.length(), username.substr(0, std::min((size_t)5, username.length())).c_str());
    ESP_LOGI(TAG, "  password: len=%d, val=%s...", (int)password.length(), password.substr(0, std::min((size_t)5, password.length())).c_str());
    ESP_LOGI(TAG, "  publish_topic: %s", publish_topic_.c_str());
    ...
    if (!mqtt_->Connect(broker_address, broker_port, client_id, username, password)) {
        ...
        ESP_LOGI(TAG, "Schedule reconnect in %d seconds", MQTT_RECONNECT_INTERVAL_MS / 1000);
        esp_timer_start_once(reconnect_timer_, MQTT_RECONNECT_INTERVAL_MS * 1000);
        return false;
    }
