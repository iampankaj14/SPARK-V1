...
bool AfeAudioEngine::Initialize(AudioCodec* codec, int frame_duration_ms, srmodel_list_t* models_list) {
    ...
    if (false) {
        ...
        ESP_LOGI(TAG, "WakeNet wake words string: '%s'", words ? words : "(null)");
        if (words != nullptr) {
        ...
        ESP_LOGI(TAG, "WakeNet wake_words_ count: %d", (int)wake_words_.size());
        for (size_t i = 0; i < wake_words_.size(); i++) {
            ESP_LOGI(TAG, "  wake_words_[%d] = '%s'", (int)i, wake_words_[i].c_str());
        }
    ...
    StackType_t* afe_stack = (StackType_t*)heap_caps_malloc(4096 * sizeof(StackType_t), MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    StaticTask_t* afe_tcb = (StaticTask_t*)heap_caps_malloc(sizeof(StaticTask_t), MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
    BaseType_t task_created = pdFAIL;
    if (afe_stack && afe_tcb) {
...
void AfeAudioEngine::EnableWakeWordDetection(bool enable) {
    ESP_LOGI(TAG, "EnableWakeWordDetection(%s), HasWakeWord=%d, wake_detector_=%d",
        enable ? "true" : "false", HasWakeWord(), (int)wake_detector_);
    if (!HasWakeWord()) {
        ESP_LOGW(TAG, "EnableWakeWordDetection: no wake word, returning early!");
        return;
    }
    ...
    if (enable) {
        ...
        ESP_LOGI(TAG, "kWakeWordEnabled bit SET");
    } else {
        xEventGroupClearBits(event_group_, kWakeWordEnabled);
        if (wake_detector_ == WakeDetector::kMultiNet) {
    ...
    UpdateActiveState();
    ESP_LOGI(TAG, "After UpdateActiveState: kAfeActive=%d",
        (xEventGroupGetBits(event_group_) & kAfeActive) ? 1 : 0);
}
...
void AfeAudioEngine::HandleWakeWordResult(const afe_fetch_result_t* result) {
    ...
    ESP_LOGI(TAG, "*** WAKENET_DETECTED! model_index=%d, wake_words_.size()=%d",
        result->wakenet_model_index, (int)wake_words_.size());

    int model_index = result->wakenet_model_index - 1;
    if (model_index < 0 || model_index >= static_cast<int>(wake_words_.size())) {
        ESP_LOGE(TAG, "Invalid WakeNet model index: %d (wake_words_.size()=%d)",
            result->wakenet_model_index, (int)wake_words_.size());
        return;
    }
